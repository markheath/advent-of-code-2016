...
let move state instruction =
    ...
    | Turn direction ->
        let nowFacing = match state.facing, direction with
                        | North, Left -> West
                        | North, Right -> East
                        | West, Left -> South
                        | West, Right -> North
...
let visitedTwice = instructions 
                    |> Array.collect expandInstruction 
                    |> Array.scan move startState
                    |> Array.find (fun f -> f.visited.Contains(f.position))
